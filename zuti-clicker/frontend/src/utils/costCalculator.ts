...
export function getBulkCost(
  ...
  //geometric series: effectiveBase * growth^owned * (growth^amount - 1) / (growth - 1)
  return (
...
export function getMaxBuyable(
  ...
  // actually costs.
  const firstCost = getBulkCost(unit, owned, 1, costMultiplier);
  if (tokens < firstCost) return 0;
  const { baseCost, costGrowth } = unit;
  let n = Math.floor(
    Math.log(
      1 + (tokens * (costGrowth - 1)) / (baseCost * costMultiplier * Math.pow(costGrowth, owned))
    ) / Math.log(costGrowth)
  );
  //two-sided float correction: the closed form can land 1 off in either
  //direction right at an exact bulk-cost boundary
  if (getBulkCost(unit, owned, n, costMultiplier) > tokens) n--;
  else if (getBulkCost(unit, owned, n + 1, costMultiplier) <= tokens) n++;
  return Math.max(0, n);
}
